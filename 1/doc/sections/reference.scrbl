...
@defproc[#:kind "функция" (применить [функция функция?] [аргумент любой] ...
         [список список?])
         любой]{
Вызывает @racket[функция] с аргументами: сначала отдельно переданные
@racket[аргумент]ы, затем элементы @racket[список]. Последний аргумент
должен быть списком. Ключевые аргументы, переданные в
@racket[применить], передаются в @racket[функция].}
...
@defproc*[#:kind "функция"
          ([(не [значение любой]) булево?]
           [(не [функция функция?] [аргумент любой] ...) булево?])]{
В форме с одним аргументом возвращает @racketvalfont{истина}, если
@racket[значение] --- @racketvalfont{ложь}, и @racketvalfont{ложь} иначе.

В форме с несколькими аргументами вызывает @racket[функция] с
@racket[аргумент]ами и отрицает результат: @racket[(не ф а б)] то же,
что @racket[(не (ф а б))].}
...
@defproc[#:kind "оператор" (// [делимое целое?] [делитель целое?])
         целое?]{
Целочисленное деление @racket[делимое] на @racket[делитель]. Дробная
часть отбрасывается в сторону нуля: @racket[(10 // 3)] даёт 3,
@racket[(-10 // 3)] даёт -3.}

@defproc[#:kind "оператор" (% [делимое целое?] [делитель целое?])
         целое?]{
Остаток от целочисленного деления @racket[делимое] на
@racket[делитель]. Знак остатка совпадает со знаком @racket[делимое]:
@racket[(10 % 3)] даёт 1, @racket[(-10 % 3)] даёт -1.}
...
@defproc[#:kind "функция" (строка->байты [строка строка?]
                                         [байт-ошибки
                                          (один-из байт? #,(elem (racketvalfont "ложь")))
                                          #,(elem (racketvalfont "ложь"))]
                                         [начало целое-неотрицательное? 0]
                                         [конец целое-неотрицательное? (длина-строки строка)])
         байты?]{Преобразует отрезок строки в байты в кодировке UTF-8.
@racket[байт-ошибки] не используется: любая литера кодируется; параметр есть
для единообразия с @racket[строка->байты/местные].}

@defproc[#:kind "функция" (строка->байты/местные [строка строка?]
         [байт-ошибки
          (один-из байт? #,(elem (racketvalfont "ложь")))
          #,(elem (racketvalfont "ложь"))]
         [начало целое-неотрицательное? 0]
         [конец целое-неотрицательное? (длина-строки строка)])
         байты?]{Преобразует отрезок строки в байты в кодировке региональных
настроек. Если @racket[байт-ошибки] не ложь, он подставляется вместо литер,
которые нельзя закодировать, иначе вызывается исключение.}
...
@defproc[#:kind "функция" (написать/перенос [аргумент любой]
         [вывод порт? (текущий-порт-вывода)])
         пусто?]{
Как @racket[написать], но после значения дополнительно выводит литеру
переноса строки.}
...
@defproc[#:kind "функция" (написать-строку [строка строка?]
         [порт порт-вывода? (текущий-порт-вывода)])
         целое-неотрицательное?]{
Пишет литеры @racket[строка] в @racket[порт]. Возвращает число записанных
литер.}

@defproc[#:kind "функция" (прочитать [порт порт-ввода? (текущий-порт-ввода)])
         любой]{
Читает одно значение из @racket[порт] в том же виде, в каком его пишет
@racket[написать]. В конце потока возвращает @racket[конец-файла].}

@defproc[#:kind "функция"
         (прочитать-строку [ввод порт? (текущий-порт-ввода)]
                           [режим (одно-из 'перенос 'возврат 'перенос/возврат
                                    'любой 'любой-один)
                            'перенос])
         (один-из строка? конец-файла)]{
Читает строку из порта @racket[ввод]. Аргумент @racket[режим] определяет
разделитель строки:
@itemlist[
 (list
...
@defthing[#:kind "константа" конец-файла конец-файла?]{
Значение конца потока ввода.}

@defproc[#:kind "функция" (конец-файла? [аргумент любой]) булево?]{
Истина, если @racket[аргумент] --- @racket[конец-файла].}

@defproc[#:kind "функция" (открыть-запись-в-строку
         [имя символ? 'строка])
         порт-вывода?]{
Порт вывода, пишущий в строку. Имя порта --- @racket[имя].}

@defproc[#:kind "функция" (получить-записанную-строку [порт порт-вывода?])
         строка?]{
Строка, накопленная портом из @racket[открыть-запись-в-строку].}

@defproc[#:kind "функция" (открыть-чтение-из-строки [строка строка?]
         [имя символ? 'строка])
         порт-ввода?]{
Порт ввода из содержимого @racket[строка].}

@defproc[#:kind "функция" (открыть-чтение-из-байтов [байты байты?]
         [имя символ? 'строка])
         порт-ввода?]{
Порт ввода из содержимого @racket[байты].}

@defproc[#:kind "функция" (получить-записанные-байты [порт порт-вывода?])
         байты?]{
Байты, накопленные портом записи в байты.}

@defproc[#:kind "функция" (немедленно-записать [порт порт-вывода?]) пусто?]{
Сбрасывает буфер @racket[порт] на устройство.}

@defproc[#:kind "функция" (закрыть-порт-ввода [порт порт-ввода?]) пусто?]{
Закрывает порт ввода.}

@defproc[#:kind "функция" (закрыть-порт-вывода [порт порт-вывода?]) пусто?]{
Закрывает порт вывода.}
...
@defproc[#:kind "функция" (путь? [аргумент любой])
         булево?]{
Возвращает истину, если @racket[аргумент] является путём файловой
системы. Модуль @racketidfont{базовая/путь}.}

@defproc[#:kind "функция" (строка->путь [строка строка?]) путь?]{
Преобразует строку в путь. Модуль @racketidfont{базовая/путь}.}

@defproc[#:kind "функция" (путь->строка [путь путь?]) строка?]{
Преобразует путь в строку. Модуль @racketidfont{базовая/путь}.}

@defproc[#:kind "функция" (построить-путь [база (один-из строка? путь?)]
         [элемент (один-из строка? путь?)] ...)
         путь?]{
Собирает путь из @racket[база] и дополнительных @racket[элемент]ов.
Не обращается к файловой системе и не проверяет, существует ли путь.
Модуль @racketidfont{базовая/путь}.}

@defproc[#:kind "функция" (разделить-путь [путь (один-из строка? путь?)])
         любой]{
Разбирает @racket[путь] на три значения: база (путь, символ
@racket['relative] или @racketvalfont{ложь} для корня), имя последнего
элемента (путь или @racket['up] / @racket['same]) и признак, что путь
синтаксически задаёт каталог. Модуль @racketidfont{базовая/путь}.}

@defproc[#:kind "функция" (текущий-каталог) путь?]{
Текущий рабочий каталог процесса. Модуль @racketidfont{базовая/файл}.}
...
@defproc[#:kind "функция" (создать-каталог [путь (один-из строка? путь?)]
         [права целое? #o777])
         пусто?]{
Создаёт каталог по пути @racket[путь]. @racket[права] --- права доступа
(на системах, где они поддерживаются). Модуль
@racketidfont{базовая/файл}.}

@defproc[#:kind "функция" (в-каталоге
         [каталог (один-из строка? путь? #,(elem (racketvalfont "ложь")))
          #,(elem (racketvalfont "ложь"))])
         последовательность?]{
Последовательность путей файлов и подкаталогов в @racket[каталог].
Если @racket[каталог] --- @racketvalfont{ложь}, обходится текущий
каталог. Модуль @racketidfont{базовая/файл}.}

@defproc[#:kind "функция" (удалить-файл [путь (один-из строка? путь?)])
         пусто?]{
Удаляет файл по пути @racket[путь]. Если путь указывает на ссылку, удаляется сама ссылка, а не то,
на что она указывает. Модуль @racketidfont{базовая/файл}.}

@defproc[#:kind "функция" (удалить-каталог [путь (один-из строка? путь?)])
         пусто?]{
Удаляет пустой каталог по пути @racket[путь]. Модуль @racketidfont{базовая/файл}.}

@defproc[#:kind "функция" (удалить-каталог/файлы [путь (один-из строка? путь?)]
                            [#:должен-существовать? должен-существовать? булево?
                             #,(elem (racketvalfont "истина"))])
         пусто?]{
Удаляет файл или каталог по пути @racket[путь]. Каталог удаляется вместе с содержимым. Если
@racket[должен-существовать?] --- @racketvalfont{истина} и пути нет, вызывается исключение; если
@racketvalfont{ложь}, отсутствие пути не считается ошибкой. Модуль @racketidfont{базовая/файл}.}

@defproc[#:kind "функция" (читая-файл [имя (один-из строка? путь?)]
         [обработка (порт-ввода? . -> . любой)]
         [#:режим режим (один-из 'двоичный 'текстовый) 'двоичный])
         любой]{
Открывает файл @racket[имя] для чтения, вызывает @racket[обработка] с
портом ввода и закрывает порт. Результат --- значение
@racket[обработка]. Модуль @racketidfont{базовая/порт}.}

@defproc[#:kind "функция" (записывая-файл [имя (один-из строка? путь?)]
         [обработка (порт-вывода? . -> . любой)]
         [#:режим режим (один-из 'двоичный 'текстовый) 'двоичный]
         [#:если-существует если-существует
          (один-из 'ошибка 'добавить 'обновить 'заменить 'обрезать
           'обрезать/заменить)
          'ошибка]
         [#:права права целое? #o666])
         любой]{
Открывает файл @racket[имя] для записи, вызывает @racket[обработка] с
портом вывода и закрывает порт. @racket[если-существует] задаёт
поведение, если файл уже есть:
@itemlist[
 @item{@racket['ошибка] --- вызвать исключение, если файл существует
  (умолчание);}
 @item{@racket['заменить] --- удалить существующий файл (если есть) и
  создать новый;}
 @item{@racket['обрезать] --- очистить содержимое, если файл есть;
  иначе создать новый;}
 @item{@racket['обрезать/заменить] --- сначала как @racket['обрезать],
  при неудаче (например, из‑за прав) --- как @racket['заменить];}
 @item{@racket['обновить] --- открыть существующий файл без очистки;
  если файла нет, вызвать исключение;}
 @item{@racket['добавить] --- писать в конец файла; если файла нет,
  создать его.}
]
Модуль @racketidfont{базовая/порт}.}
...
@section[#:tag "date"]{Дата}

Модуль @racketidfont{дата} --- календарная дата и время суток:

@codeblock|{
используется дата
д = дата 2026 9 22 15 30
дата->строка д
дата->строка $ начало-дня д
}|

Имена @racket[дата], @racket[год], @racket[час] совпадают с модулем
@racketidfont{база-данных}: подключайте один из модулей с префиксом.

@defthing[#:kind "структура" дата% любой]{
Календарная дата и время суток. Поля: @racket[секунда], @racket[минута],
@racket[час], @racket[день], @racket[месяц], @racket[год],
@racket[день-недели], @racket[день-года], @racket[летнее-время?],
@racket[временная-зона] и @racket[наносекунда]. Структура читаемая.}

@defproc[#:kind "функция" (дата [год целое? 1] [месяц целое? 1] [день целое? 1]
         [час целое? 0] [минута целое? 0] [секунда целое? 0]
         [наносекунда целое? 0])
         дата%]{
Собирает дату. День недели, день года, летнее время и зона считаются
по местному часовому поясу.}

@defproc*[#:kind "функция"
          ([(секунда [дата дата%]) целое?]
           [(минута [дата дата%]) целое?]
           [(час [дата дата%]) целое?]
           [(день [дата дата%]) целое?]
           [(месяц [дата дата%]) целое?]
           [(год [дата дата%]) целое?]
           [(наносекунда [дата дата%]) целое?]
           [(день-недели [дата дата%]) целое?]
           [(день-года [дата дата%]) целое?]
           [(летнее-время? [дата дата%]) булево?]
           [(временная-зона [дата дата%]) целое?])]{
Селекторы полей @racket[дата%]. @racket[день-недели]: 0 --- воскресенье,
6 --- суббота. @racket[день-года]: 0 --- 1 января.
@racket[временная-зона] --- смещение в секундах к востоку от GMT.
@racket[летнее-время?] --- @racketvalfont{истина}, если действует летнее
время.}

@defproc[#:kind "функция" (секунды->дата [секунды число?]) дата%]{
Дата по числу секунд с 1 января 1970 года (UTC) в местном часовом поясе.}

@defproc[#:kind "функция" (дата->секунды [дата дата%]) целое?]{
Число секунд с 1 января 1970 года (UTC).}

@defproc[#:kind "функция" (дата->строка [дата дата%]) строка?]{
Строка @litchar{ДД.ММ.ГГГГ ЧЧ:ММ:СС}. Если наносекунды не нуль, они
добавляются после точки девятью цифрами.}

@defproc[#:kind "функция" (начало-дня [дата дата%]) дата%]{
Копия с временем 00:00:00.}

@defproc[#:kind "функция" (конец-дня [дата дата%]
         [совсем булево? #,(elem (racketvalfont "ложь"))])
         дата%]{
Копия с временем 23:59:59. Если @racket[совсем] ---
@racketvalfont{истина}, наносекунды 999999999, иначе 0.}

@defproc[#:kind "функция" (начало-месяца [дата дата%]) дата%]{
Первый день того же месяца, время 00:00:00.}

@defproc[#:kind "функция" (конец-месяца [дата дата%]
         [совсем булево? #,(elem (racketvalfont "ложь"))])
         дата%]{
Последний день того же месяца. Время как у @racket[конец-дня].}

@defproc[#:kind "функция" (начало-года [дата дата%]) дата%]{
1 января того же года, время 00:00:00.}

@defproc[#:kind "функция" (конец-года [дата дата%]) дата%]{
31 декабря того же года. Время как у @racket[конец-дня] без
@racket[совсем].}

@defproc[#:kind "функция" (добавить-месяц [дата дата%] [количество целое?])
         дата%]{
Сдвигает месяц на @racket[количество] (отрицательное --- назад). Если
такого дня в новом месяце нет, берётся последний день месяца.}
...
@defproc[#:kind "метод" (установить
         [красный целое-неотрицательное?]
         [зелёный целое-неотрицательное?]
         [синий целое-неотрицательное?]
         [укрывистость число?])
         пусто?]{
...
без аргументов возвращает два значения --- масштаб по ширине и по высоте; с @racket[по-ширине] и
@racket[по-высоте] задаёт его. Это абсолютный масштаб логических координат;
@racket[масштабировать] умножает текущее преобразование.}

@defproc[#:kind "метод" (масштаб-подложки)
         число?]{
масштаб подложки холста: сколько пикселей на одну единицу рисования.}

@defproc[#:kind "метод" (высота-литеры)
         число?]{
высота литеры текущего шрифта.}

@defproc[#:kind "метод" (ширина-литеры)
         число?]{
средняя ширина литеры текущего шрифта.}

@defproc*[#:kind "метод"
          ([(область-отсечения) любой]
           [(область-отсечения [значение любой]) пусто?])]{
без аргумента --- текущая область отсечения или @racketvalfont{ложь}; с аргументом задаёт её.
@racketvalfont{ложь} снимает отсечение. См. также @racket[отсечь-прямоугольник].}

@defproc[#:kind "метод" (отсечь-прямоугольник [лево число?] [верх число?]
                        [ширина число?] [высота число?])
         пусто?]{
задаёт прямоугольную область отсечения.}

@defproc[#:kind "метод" (масштаб-устройства)
         любой]{
два значения: масштаб устройства по ширине и по высоте.}

@defproc*[#:kind "метод"
          ([(шрифт) любой]
           [(шрифт [значение любой]) пусто?])]{
без аргумента --- текущий шрифт; с аргументом задаёт его.}

@defproc[#:kind "метод" (контекст-opengl)
         любой]{
контекст OpenGL или @racketvalfont{ложь}, если его нет.}

@defproc*[#:kind "метод"
          ([(начальная-матрица) любой]
           [(начальная-матрица [матрица любой]) пусто?])]{
без аргумента --- начальная матрица преобразования (вектор из шести чисел); с аргументом задаёт её.}

@defproc*[#:kind "метод"
          ([(начало-координат) любой]
           [(начало-координат [лево число?] [верх число?]) пусто?])]{
без аргументов возвращает начало логических координат в координатах устройства; с @racket[лево] и
@racket[верх] задаёт его. @racket[сдвинуть] добавляет сдвиг к текущему преобразованию.}

@defproc*[#:kind "метод"
          ([(перо) любой]
           [(перо [значение любой]) пусто?]
           [(перо [цвет любой] [толщина число?] [стиль любой]) пусто?])]{
без аргументов возвращает текущее перо; с пером задаёт его; с цветом, толщиной и стилем создаёт перо.
Цвет может быть символом из @racket[стандартные-цвета]. Стиль: @racket['прозрачный],
@racket['сплошной], @racket['исключающее-или], @racket['подсветка], @racket['точки],
@racket['длинный-пунктир], @racket['короткий-пунктир], @racket['штрих-пунктир] и те же с приставкой
@racketidfont{исключающее-или-}.}

@defproc[#:kind "метод" (рамка-контура [контур любой] [тип символ?])
         любой]{
четыре значения: лево, верх, ширина и высота рамки контура. @racket[тип] --- @racket['контур]
(точки пути), @racket['обводка] (с учётом пера) или @racket['заливка].}

@defproc*[#:kind "метод"
          ([(поворот) число?]
           [(поворот [угол число?]) пусто?])]{
без аргумента --- угол поворота логических координат в радианах; с аргументом задаёт его.
@racket[повернуть] добавляет поворот к текущему преобразованию.}

@defproc[#:kind "метод" #:link-target? #f (размер)
         любой]{
два значения: ширина и высота области рисования.}

@defproc*[#:kind "метод" #:link-target? #f
          ([(сглаживание) символ?]
           [(сглаживание [режим символ?]) пусто?])]{
без аргумента --- режим сглаживания; с аргументом задаёт его. Значения: @racket['без-сглаживания],
@racket['сглаженное], @racket['выровненное].}

@defproc*[#:kind "метод"
          ([(цвет-фона-текста) любой]
           [(цвет-фона-текста [цвет любой]) пусто?])]{
цвет фона текста (чтение и запись). Учитывается в режиме @racket['сплошной] у
@racket[режим-текста].}

@defproc[#:kind "метод" (размеры-текста [текст строка?]
                  [шрифт любой #,(elem (racketvalfont "ложь"))]
                  [комбинировать любой #,(elem (racketvalfont "ложь"))]
                  [начальная-литера целое? 0])
         любой]{
четыре значения: ширина текста, высота шрифта, спуск под базовую линию и дополнительный вертикальный
зазор. Если @racket[шрифт] --- @racketvalfont{ложь}, берётся текущий шрифт. @racket[комбинировать]
как у @racket[нарисовать-текст].}

@defproc*[#:kind "метод"
          ([(режим-текста) символ?]
           [(режим-текста [режим символ?]) пусто?])]{
без аргумента --- режим рисования текста; с аргументом задаёт его. @racket['сплошной] заливает фон
цветом @racket[цвет-фона-текста]; @racket['прозрачный] рисует поверх уже нарисованного.}

@defproc*[#:kind "метод"
          ([(преобразование) любой]
           [(преобразование [значение любой]) пусто?])]{
без аргумента --- полное преобразование холста; с аргументом задаёт его. @racket[преобразовать]
умножает текущее преобразование на матрицу.}

@defproc[#:kind "метод" #:link-target? #f (есть-литера? [литера любой])
         булево?]{
@racketvalfont{истина}, если для литеры есть знак.}

@defproc[#:kind "метод" #:link-target? #f (работает?)
         булево?]{
@racketvalfont{истина}, если холст пригоден для рисования.}

@defproc[#:kind "метод" #:link-target? #f (продолжить-отрисовку)
         пусто?]{
возобновляет сброс отложенной отрисовки картины; для прочих холстов ничего не делает.}

@defproc[#:kind "метод" #:link-target? #f (приостановить-отрисовку)
         пусто?]{
откладывает сброс отрисовки картины; для прочих холстов ничего не делает.}

@defproc[#:kind "метод" (повернуть [угол число?])
         пусто?]{
добавляет поворот на @racket[угол] радиан к текущему преобразованию.}

@defproc[#:kind "метод" (масштабировать [по-ширине число?] [по-высоте число?])
         пусто?]{
умножает текущее преобразование на масштаб.}

@defproc[#:kind "метод" (масштаб-выравнивания [значение число?])
         пусто?]{
масштаб выравнивания координат при рисовании без сглаживания или в выровненном режиме. По умолчанию
1.}

@defproc[#:kind "метод" (преобразовать [матрица любой])
         пусто?]{
умножает текущее преобразование на матрицу из шести чисел.}

@defproc[#:kind "метод" (сдвинуть [вправо число?] [вниз число?])
         пусто?]{
добавляет сдвиг к текущему преобразованию.}

@defproc[#:kind "метод" (подобрать-цвет [проба любой] [результат любой])
         пусто?]{
записывает в @racket[результат] цвет, которым холст реально нарисует @racket[проба].}
...
@defproc[#:kind "функция" (воспроизвести-звук [файл (один-из строка? путь?)]
         ...
         булево?]{
Воспроизводит звуковой файл. Если
...
Подключение к узлу, обмен пакетами и постоянная связь. Модули
подключаются по имени:

@itemlist[
 @item{@racketidfont{надёжное-соединение} --- поток с гарантией
  доставки и порядка;}
 @item{@racketidfont{ненадёжное-соединение} --- пакеты без гарантии
  доставки и порядка;}
 @item{@racketidfont{постоянное-соединение} --- сериализуемые значения
  Адины и восстановление связи при обрыве.}
]

@subsection[#:tag "ref-надёжное-соединение"]{@racketidfont{надёжное-соединение}}

Подключение к узлу сети: порты ввода и вывода.
...
Результат передают в @racket[принять-соединение].}
...
Возвращает порт ввода и порт вывода.}

@subsection[#:tag "ref-ненадёжное-соединение"]{@racketidfont{ненадёжное-соединение}}

Обмен пакетами без гарантии доставки и порядка.
...
потери пакетов при редком опросе.}
...
@subsection[#:tag "ref-постоянное-соединение"]{@racketidfont{постоянное-соединение}}

Клиент при обрыве подключается снова; сообщения подтверждаются.
Данные должны быть такими, чтобы @racket[написать] вывел их в
читаемом обратно виде (строки, числа, списки, массивы). Функции,
объекты и @racketvalfont{пусто} так передать нельзя: тогда придёт
строка, которую дал @racket[написать].

@defproc[#:kind "функция" #:link-target? #f (соединить [имя-сервера строка?]
         [номер-порта целое-неотрицательное?]
         [#:при-подключении при-подключении функция?
          (функция (причина-отключения соединение) пусто)]
         [#:при-получении при-получении функция?
          (функция (объект) пусто)]
         [#:при-изменении-состояния при-изменении-состояния функция?
          (функция (состояние) пусто)])
...
завершения, как @racket[соединить].}

@defproc[#:kind "функция" #:link-target? #f
         (отправить [соединение любой] [данные любой])
         пусто?]{
Отправляет @racket[данные] в @racket[соединение], полученное от
@racket[соединить] или @racket[принять-соединение].}

@section[#:tag "почта"]{Почта}

Модуль @racketidfont{почта}. Отправка писем через SMTP.

@defproc[#:kind "функция" (отправить-почту [имя-сервера строка?]
         [отправитель строка?]
         [получатели (список-из строка?)]
         [заголовок строка?]
         [сообщение (список-из (один-из строка? байты?))]
         [#:номер-порта номер-порта целое-неотрицательное? 25]
         ...
         [#:пароль пароль
          (один-из строка? #,(elem (racketvalfont "ложь")))
          #,(elem (racketvalfont "ложь"))]
         [#:oauth2? oauth2? любой #,(elem (racketvalfont "ложь"))]
         [#:подключиться подключиться функция? соединить]
         [#:шифрование шифрование
          (один-из функция? #,(elem (racketvalfont "ложь")))
          #,(elem (racketvalfont "ложь"))])
         пусто?]{
Подключается к серверу @racket[имя-сервера] и отправляет письмо.
@racket[отправитель] --- адрес отправителя. @racket[получатели] ---
список адресов (поля «Кому», копия и скрытая копия).
@racket[заголовок] --- полный заголовок письма, уже содержащий поля
отправителя, получателей и копии, согласованные с @racket[отправитель]
и @racket[получатели]. @racket[сообщение] --- тело: список строк или
байтовых строк; каждый элемент --- одна строка текста без перевода
строки.

@racket[номер-порта] по умолчанию 25. @racket[пользователь] и
@racket[пароль] задают вход на сервер. Если @racket[oauth2?] не
@racketvalfont{ложь}, вместо пароля передают токен доступа.

@racket[подключиться] получает имя сервера и номер порта и возвращает
порты ввода и вывода; по умолчанию как @racket[соединить] надёжного
соединения. Если @racket[шифрование] не @racketvalfont{ложь}, после
приветствия канал защищают этой функцией. Обычно задают либо
защищённое @racket[подключиться], либо @racket[шифрование], а не оба
сразу.
Модуль @racketidfont{почта}.}

@section[#:tag "hypertext-client"]{Клиент гипертекста}

Модуль @racketidfont{клиент-гипертекста}. Запрос к узлу по HTTP
и получение ответа.

@defproc[#:kind "функция" (отправить-получить-гипертекст
         [узел (один-из байты? строка?)]
         [путь (один-из байты? строка?)]
         [#:защищённый? защищённый? любой
          #,(elem (racketvalfont "ложь"))]
         [#:порт порт целое-неотрицательное?
          (если защищённый? тогда 443 иначе 80)]
         [#:версия версия (один-из байты? строка?) #"1.1"]
         [#:метод метод любой 'получение]
         [#:заголовки заголовки список? пустой-список]
         [#:данные данные
          (один-из байты? строка? функция?
           #,(elem (racketvalfont "ложь")))
          #,(elem (racketvalfont "ложь"))]
         [#:декодирование декодирование список?
          (список 'gzip 'deflate)])
         (values байты? список? порт-ввода?)]{
Открывает соединение с @racket[узел], отправляет запрос к @racket[путь]
и возвращает три значения: строку состояния (байты), список заголовков
ответа (байты) и порт ввода тела.

Если @racket[защищённый?] не @racketvalfont{ложь}, соединение шифруют;
порт по умолчанию тогда 443, иначе 80. @racket[метод] ---
@racket['получение] или @racket['GET] (получение), @racket['отправка]
или @racket['POST] (отправка); иные значения передают как есть.
@racket[данные] --- тело запроса. @racket[декодирование] --- символы
способов сжатия тела ответа; по умолчанию @racket['gzip] и
@racket['deflate].}

@section[#:tag "json-format"]{Формат JSON}

Модуль @racketidfont{формат-json}. Имена короткие, поэтому модуль
подключают с префиксом:

@codeblock|{
используется
  с-префиксом json: формат-json
}|

@defproc[#:kind "функция" #:link-target? #f (прочитать
         [порт порт-ввода? (текущий-порт-ввода)]
         [#:пусто пусто любой]
         [#:заменять-неверный-сурогат? заменять-неверный-сурогат? любой
          #,(elem (racketvalfont "ложь"))])
         любой]{
Читает одно JSON-значение из @racket[порт]. Если остались только
пробелы, результат --- @racket[конец-файла]. @racket[пусто] --- значение,
которым представляют JSON null (по умолчанию символ @racket['null]).
Если @racket[заменять-неверный-сурогат?] не @racketvalfont{ложь},
неверный суррогат заменяют литерой замены Юникода, иначе вызывается
исключение.}

@defproc[#:kind "функция" (из-строки [строка строка?]
         [#:пусто пусто любой]
         [#:заменять-неверный-сурогат? заменять-неверный-сурогат? любой
          #,(elem (racketvalfont "ложь"))])
         любой]{
Разбирает JSON из префикса @racket[строка]. @racket[пусто] и
@racket[заменять-неверный-сурогат?] --- как при чтении из порта.}

@defproc[#:kind "функция" #:link-target? #f (в-строку [значение любой]
         [#:пусто пусто любой]
         [#:кодирование кодирование любой 'управляющие]
         [#:отступ отступ
          (один-из литера? целое-неотрицательное?
           #,(elem (racketvalfont "ложь")))
          #,(elem (racketvalfont "ложь"))])
         строка?]{
Собирает JSON-текст из @racket[значение]. @racket[кодирование] ---
@racket['управляющие] (экранировать управляющие литеры) или
@racket['все] (экранировать все не-ASCII). @racket[отступ] ---
@racketvalfont{ложь} (без форматирования), литера табуляции или число
пробелов.}

@section[#:tag "database"]{База данных}

Модуль @racketidfont{база-данных} --- запросы, транзакции, типы значений
и пул подключений. Модуль @racketidfont{база-данных-mysql} добавляет
подключение к MySQL и перепредоставляет общий модуль, поэтому достаточно:
...
подключение = подключить
  ...
  #:база «app»
  #:пароль «secret»
строки = строки-запроса подключение «select id, name from t where id = ?» 42
отключить подключение
}|

Имена @racket[дата], @racket[время], @racket[год], @racket[час] короткие.
Вместе с модулем @racketidfont{дата} подключайте @racketidfont{база-данных}
с префиксом.

В тексте запроса MySQL плейсхолдер параметра --- @litchar{?}.

@subsection[#:tag "database-mysql"]{@racketidfont{база-данных-mysql}}

@defproc[#:kind "функция" (подключить
         [#:пользователь пользователь строка?]
         [#:база база
          (один-из строка? #,(elem (racketvalfont "ложь")))
          #,(elem (racketvalfont "ложь"))]
         [#:сервер сервер строка? "localhost"]
         [#:порт порт целое-неотрицательное? 3306]
         [#:сокет сокет
          (один-из строка? путь? 'угадать
           #,(elem (racketvalfont "ложь")))
          #,(elem (racketvalfont "ложь"))]
         [#:пароль пароль
          (один-из строка? #,(elem (racketvalfont "ложь")))
          #,(elem (racketvalfont "ложь"))]
         [#:открытый-пароль? открытый-пароль? любой 'местно]
         [#:шифрование шифрование любой 'нет]
         [#:контекст-шифрования контекст-шифрования любой
          #,(elem (racketvalfont "ложь"))]
         [#:обработчик-уведомлений обработчик-уведомлений любой пусто])
         подключение?]{
Подключается к серверу MySQL. Если @racket[база] ---
@racketvalfont{ложь}, текущую базу задают позже командой SQL
@litchar{USE}. @racket[сокет] --- путь к локальному сокету; символ
@racket['угадать] выбирает обычный путь. Нельзя одновременно задать
@racket[сокет] и @racket[сервер]/@racket[порт].

@racket[шифрование]: @racket['нет], @racket['да] или
@racket['необязательно]. @racket[открытый-пароль?] ---
@racket['местно], @racketvalfont{истина} или @racketvalfont{ложь}.
Если @racket[контекст-шифрования] --- @racketvalfont{ложь}, контекст
создаётся при необходимости. Если @racket[обработчик-уведомлений] ---
@racket[пусто], используется поведение по умолчанию.
Модуль @racketidfont{база-данных-mysql}.}

@defproc[#:kind "функция" (угадать-путь-сокета)
         (один-из строка? путь?)]{
Возвращает первый существующий обычный путь сокета MySQL. Если ни один
не найден, вызывается исключение.}

@subsection[#:tag "database-base"]{@racketidfont{база-данных}}

@defproc[#:kind "функция" (подключение? [значение любой]) булево?]{
Истина, если @racket[значение] --- подключение к базе данных.}

@defproc[#:kind "функция" (отключить [подключение подключение?]) пусто?]{
Закрывает @racket[подключение].}

@defproc[#:kind "функция" (подключено? [подключение подключение?])
         булево?]{
Истина, если @racket[подключение] ещё открыто.}

@defproc[#:kind "функция" (система-бд [подключение подключение?])
         система-бд?]{
Объект системы базы данных этого подключения.}

@defproc[#:kind "функция" (система-бд? [значение любой]) булево?]{
Истина, если @racket[значение] --- объект системы базы данных.}

@defproc[#:kind "функция" (имя-системы-бд [система система-бд?]) символ?]{
Символ системы, для MySQL --- @racket['mysql].}

@defproc[#:kind "функция" (типы-системы-бд [система система-бд?])
         (список-из символ?)]{
Список известных типов этой системы.}

@defproc[#:kind "функция" (устойчивое-подключение [подключение подключение?])
         подключение?]{
Прокси, при котором убийство потока во время запроса не портит
подключение.}

@defproc[#:kind "функция" (выполнить-запрос [подключение подключение?]
         [запрос запрос?] [аргумент любой] ...)
         пусто?]{
Выполняет SQL ради эффекта (вставка, обновление, удаление).}

@defproc[#:kind "функция" (строки-запроса [подключение подключение?]
         [запрос запрос?] [аргумент любой] ...
         [#:группировка группировка любой пустой-список]
         [#:режим-группировки режим-группировки список? пустой-список])
         (список-из массив?)]{
Возвращает список строк (массивов). @racket[режим-группировки] ---
список символов @racket['список] и @racket['сохранить-нуль].}

@defproc[#:kind "функция" (список-запроса [подключение подключение?]
         [запрос запрос?] [аргумент любой] ...)
         список?]{
Запрос из одного столбца: список значений.}

@defproc[#:kind "функция" (строка-запроса [подключение подключение?]
         [запрос запрос?] [аргумент любой] ...)
         массив?]{
Ровно одна строка результата.}

@defproc[#:kind "функция" (возможная-строка-запроса
         [подключение подключение?]
         [запрос запрос?] [аргумент любой] ...)
         (один-из массив? #,(elem (racketvalfont "ложь")))]{
Как @racket[строка-запроса], но ноль строк дают @racketvalfont{ложь}.}

@defproc[#:kind "функция" (значение-запроса [подключение подключение?]
         [запрос запрос?] [аргумент любой] ...)
         любой]{
Ровно одна строка и один столбец: само значение.}

@defproc[#:kind "функция" (возможное-значение-запроса
         [подключение подключение?]
         [запрос запрос?] [аргумент любой] ...)
         любой]{
Как @racket[значение-запроса], но ноль строк дают @racketvalfont{ложь}.}

@defproc[#:kind "функция" (запрос [подключение подключение?]
         [запрос запрос?] [аргумент любой] ...)
         (один-из простой-результат? результат-строк?)]{
Общий запрос: либо эффект, либо строки с заголовками.}

@defproc[#:kind "функция" (в-запросе [подключение подключение?]
         [запрос запрос?] [аргумент любой] ...
         [#:пачка пачка любой]
         [#:группировка группировка любой пустой-список]
         [#:режим-группировки режим-группировки список? пустой-список])
         последовательность?]{
Последовательность строк для цикла. По умолчанию все строки читаются
сразу; конечная @racket[пачка] читает порциями.}

@defproc[#:kind "функция" (запрос? [значение любой]) булево?]{
Истина, если @racket[значение] можно выполнить как запрос (строка SQL,
подготовленный или виртуальный запрос).}

@defproc[#:kind "функция" (простой-результат [сведения список?])
         простой-результат?]{
Результат команды без строк. В @racket[сведения] бывают ключи
@racket['insert-id] и @racket['affected-rows].}

@defproc[#:kind "функция" (простой-результат? [значение любой])
         булево?]{
Истина, если @racket[значение] --- простой результат.}

@defproc[#:kind "функция" (сведения-простого-результата
         [значение простой-результат?])
         список?]{
Ассоциативный список сведений.}

@defproc[#:kind "функция" (результат-строк [заголовки список?]
         [строки (список-из массив?)])
         результат-строк?]{
Результат выборки: заголовки столбцов и строки.}

@defproc[#:kind "функция" (результат-строк? [значение любой])
         булево?]{
Истина, если @racket[значение] --- результат со строками.}

@defproc[#:kind "функция" (заголовки-результата-строк
         [значение результат-строк?])
         список?]{
Заголовки столбцов.}

@defproc[#:kind "функция" (строки-результата-строк
         [значение результат-строк?])
         (список-из массив?)]{
Строки результата.}

@defproc[#:kind "функция" (сгруппировать-строки [результат результат-строк?]
         [#:группировка группировка любой]
         [#:режим-группировки режим-группировки список? пустой-список])
         результат-строк?]{
Группирует строки результата по полям @racket[группировка].}

@defproc[#:kind "функция" (строки->соответствие [результат результат-строк?]
         [#:ключ ключ любой]
         [#:значение значение любой]
         [#:режим-значения режим-значения список? пустой-список])
         соответствие?]{
Строит соответствие из строк: ключ и значение --- имена или номера
полей.}

@defproc[#:kind "функция" (подготовить [подключение подключение?]
         [запрос (один-из строка? виртуальный-запрос?)])
         подготовленный-запрос?]{
Готовит запрос на стороне сервера. Результат привязан к этому
подключению.}

@defproc[#:kind "функция" (подготовленный-запрос? [значение любой])
         булево?]{
Истина, если @racket[значение] получено из @racket[подготовить].}

@defproc[#:kind "функция" (типы-параметров-подготовленного-запроса
         [запрос подготовленный-запрос?])
         список?]{
Описание типов параметров.}

@defproc[#:kind "функция" (типы-результата-подготовленного-запроса
         [запрос подготовленный-запрос?])
         список?]{
Описание типов столбцов результата.}

@defproc[#:kind "функция" (связать-подготовленный-запрос
         [запрос подготовленный-запрос?]
         [параметры список?])
         связанный-запрос?]{
Связывает параметры; дальше передаётся в функции запроса.}

@defproc[#:kind "функция" (связанный-запрос? [значение любой])
         булево?]{
Истина, если @racket[значение] --- связанный подготовленный запрос.}

@defproc[#:kind "функция" (виртуальный-запрос
         [описание (один-из строка? функция?)])
         виртуальный-запрос?]{
Запрос, который готовится для каждого подключения при первом
исполнении. @racket[описание] --- SQL или функция от системы БД.}

@defproc[#:kind "функция" (виртуальный-запрос? [значение любой])
         булево?]{
Истина, если @racket[значение] создано @racket[виртуальный-запрос].}

@defproc[#:kind "функция" (начать-транзакцию [подключение подключение?]
         [#:изоляция изоляция любой #,(elem (racketvalfont "ложь"))]
         [#:параметр параметр любой #,(elem (racketvalfont "ложь"))])
         пусто?]{
Начинает транзакцию. @racket[изоляция]: @racket['сериализуемая],
@racket['повторяемое-чтение], @racket['чтение-подтверждённое],
@racket['чтение-неподтверждённое] или @racketvalfont{ложь}. У MySQL
@racket[параметр] не используется.}

@defproc[#:kind "функция" (зафиксировать-транзакцию
         [подключение подключение?])
         пусто?]{
Фиксирует текущую транзакцию, если она открыта.}

@defproc[#:kind "функция" (откатить-транзакцию [подключение подключение?])
         пусто?]{
Откатывает текущую транзакцию, если она открыта.}

@defproc[#:kind "функция" (в-транзакции? [подключение подключение?])
         булево?]{
Истина, если на подключении открыта транзакция.}

@defproc[#:kind "функция" (нужен-откат? [подключение подключение?])
         булево?]{
Истина, если транзакция испорчена и её нужно откатить.}

@defproc[#:kind "функция" (вызывая-транзакцию [подключение подключение?]
         [обработка функция?]
         [#:изоляция изоляция любой #,(elem (racketvalfont "ложь"))]
         [#:параметр параметр любой #,(elem (racketvalfont "ложь"))])
         любой]{
Вызывает @racket[обработка] в транзакции. При нормальном завершении
фиксирует, при исключении откатывает.}

@defproc[#:kind "функция" (список-таблиц [подключение подключение?]
         [#:схема схема любой 'поиск-или-текущая])
         (список-из строка?)]{
Имена таблиц и представлений. @racket[схема]: @racket['поиск],
@racket['текущая] или @racket['поиск-или-текущая].}

@defproc[#:kind "функция" (существует-таблица? [подключение подключение?]
         [имя строка?]
         [#:схема схема любой 'поиск-или-текущая]
         [#:учитывать-регистр? учитывать-регистр? любой
          #,(elem (racketvalfont "ложь"))])
         булево?]{
Истина, если таблица или представление @racket[имя] существует.}

@defproc[#:kind "функция" (пул-подключений [создать функция?]
         [#:наибольше-подключений наибольше-подключений любой]
         [#:наибольше-простоя наибольше-простоя любой 10]
         [#:секунды-простоя секунды-простоя вещественное? 300])
         пул-подключений?]{
Пул. @racket[создать] без аргументов возвращает новое подключение.}

@defproc[#:kind "функция" (пул-подключений? [значение любой])
         булево?]{
Истина, если @racket[значение] --- пул подключений.}

@defproc[#:kind "функция" (взять-из-пула [пул пул-подключений?]
         [освобождение любой]
         [#:таймаут таймаут
          (один-из вещественное? #,(elem (racketvalfont "ложь")))
          #,(elem (racketvalfont "ложь"))]
         [#:если-нет если-нет любой])
         подключение?]{
Берёт подключение из пула. По умолчанию @racket[освобождение] ---
текущий поток: при его завершении подключение возвращается.
@racket[отключить] тоже возвращает его в пул.}

@defproc[#:kind "функция" (виртуальное-подключение [создать любой])
         подключение?]{
Подключение по требованию: @racket[создать] --- функция без аргументов
или пул.}

@defthing[#:kind "константа" нуль любой]{
Значение SQL NULL. Это не @racket[пусто].}

@defproc[#:kind "функция" (нуль? [значение любой]) булево?]{
Истина, если @racket[значение] --- @racket[нуль].}

@defproc[#:kind "функция" (нуль->ложь [значение любой]) любой]{
Если @racket[значение] --- @racket[нуль], результат
@racketvalfont{ложь}, иначе само значение.}

@defproc[#:kind "функция" (ложь->нуль [значение любой]) любой]{
Если @racket[значение] --- @racketvalfont{ложь}, результат
@racket[нуль], иначе само значение.}

@defproc[#:kind "функция" #:link-target? #f (дата [год целое?]
         [месяц целое?] [день целое?])
         дата?]{
Дата SQL (год, месяц, день). Селекторы @racket[год], @racket[месяц],
@racket[день]. Для MySQL месяц или день могут быть нулём.}

@defproc[#:kind "функция" (дата? [значение любой]) булево?]{
Истина, если @racket[значение] --- дата SQL.}

@defproc[#:kind "функция" #:link-target? #f (время [час целое?]
         [минута целое?] [секунда целое?] [наносекунда целое?]
         [зона (один-из целое? #,(elem (racketvalfont "ложь")))])
         время?]{
Время SQL. Селекторы @racket[час], @racket[минута], @racket[секунда],
@racket[наносекунда], @racket[зона] (смещение в секундах к востоку от
GMT или @racketvalfont{ложь}).}

@defproc[#:kind "функция" (время? [значение любой]) булево?]{
Истина, если @racket[значение] --- время SQL.}

@defproc[#:kind "функция" (метка-времени [год целое?] [месяц целое?]
         [день целое?] [час целое?] [минута целое?] [секунда целое?]
         [наносекунда целое?]
         [зона (один-из целое? #,(elem (racketvalfont "ложь")))])
         метка-времени?]{
Метка времени SQL. Селекторы @racketidfont{год-метки-времени},
@racketidfont{месяц-метки-времени}, @racketidfont{день-метки-времени},
@racketidfont{час-метки-времени}, @racketidfont{минута-метки-времени},
@racketidfont{секунда-метки-времени},
@racketidfont{наносекунда-метки-времени},
@racketidfont{зона-метки-времени}.}

@defproc[#:kind "функция" (метка-времени? [значение любой]) булево?]{
Истина, если @racket[значение] --- метка времени SQL.}

@defproc[#:kind "функция" #:link-target? #f (интервал [годы целое?]
         [месяцы целое?] [дни целое?] [часы целое?] [минуты целое?]
         [секунды целое?] [наносекунды целое?])
         интервал?]{
Интервал SQL. Селекторы @racket[годы], @racket[месяцы], @racket[дни],
@racket[часы], @racket[минуты], @racket[секунды],
@racket[наносекунды].}

@defproc[#:kind "функция" (интервал? [значение любой]) булево?]{
Истина, если @racket[значение] --- интервал SQL.}

@defproc[#:kind "функция" (интервал-год-месяц? [значение любой])
         булево?]{
Истина, если интервал задаёт только годы и месяцы.}

@defproc[#:kind "функция" (интервал-день-время? [значение любой])
         булево?]{
Истина, если интервал задаёт только дни и время суток.}

@defproc[#:kind "функция" (интервал->время [интервал интервал?])
         время?]{
Переводит интервал «время суток» во @racket[время].}

@defproc[#:kind "функция" (время->интервал [время время?]) интервал?]{
Переводит @racket[время] в интервал.}

@defproc[#:kind "функция" (дата-из-базы-данных [значение дата?]) дата%]{
Дата SQL в @racket[дата%] (время 00:00:00).}

@defproc[#:kind "функция" (дата-в-базу-данных [значение дата%]) дата?]{
@racket[дата%] в дату SQL (только год, месяц, день).}

@defproc[#:kind "функция" (метка-времени-из-базы-данных
         [значение метка-времени?])
         дата%]{
Метка времени SQL в @racket[дата%]. Если зона не
@racketvalfont{ложь}, она становится временной зоной.}

@defproc[#:kind "функция" (метка-времени-в-базу-данных [значение дата%])
         метка-времени?]{
@racket[дата%] в метку времени SQL.}

@defproc[#:kind "функция" (ошибка-базы-данных? [значение любой])
         булево?]{
Истина, если @racket[значение] --- ошибка сервера базы данных.}

@defproc[#:kind "функция" (состояние-ошибки-базы-данных
         [значение ошибка-базы-данных?])
         (один-из строка? символ?)]{
Код SQLSTATE.}

@defproc[#:kind "функция" (сведения-ошибки-базы-данных
         [значение ошибка-базы-данных?])
         список?]{
Ассоциативный список сведений об ошибке. Сообщения сервера не
переводятся.}

Числа, строки и байты приходят как обычные значения Адины. SQL NULL ---
@racket[нуль]. Типы MySQL @litchar{enum} и @litchar{set} нужно
приводить к строке или числу в самом SQL.

@section[#:tag "hypertext-server"]{Сервер гипертекста}

Модуль @racketidfont{сервер-гипертекста/сервер}.

@defproc[#:kind "функция" (управляющий-ограничением-времени
         [обработчик-истечения
          (один-из функция? #,(elem (racketvalfont "ложь")))
          #,(elem (racketvalfont "ложь"))]
         [секунды-экземпляра число? 7200]
         [секунды-продолжения число? 7200])
         любой]{
Создаёт управляющего продолжениями сервлета с ограничением времени.
@racket[секунды-экземпляра] --- время жизни экземпляра сервлета,
@racket[секунды-продолжения] --- время жизни отдельного продолжения.
По истечении вызывается @racket[обработчик-истечения] (функция от
запроса, возвращающая ответ) или, если он @racketvalfont{ложь},
стандартная ошибка. Модуль @racketidfont{сервер-гипертекста/сервер}.}
...
@defproc[#:kind "функция" (сервер/единственный
         ...
         [#:время-жизни-страницы время-жизни-страницы
          (один-из число? #,(elem (racketvalfont "ложь")))
          #,(elem (racketvalfont "ложь"))]
         ...
         [#:сертификат сертификат
          (один-из путь? #,(elem (racketvalfont "ложь")))
          (&& защищённый?
              (построить-путь корень-сервера «server-cert.pem»))]
         [#:ключ ключ
          (один-из путь? #,(elem (racketvalfont "ложь")))
          (&& защищённый?
              (построить-путь корень-сервера «private-key.pem»))]
         [#:журнал журнал
...
@racket[регулярное-сервлета] по умолчанию --- точное совпадение с
@racket[путь-сервлета]. @racket[пределы-безопасности] учитывают
@racket[наибольшая-очередь]. Если @racket[время-жизни-страницы] ---
число, @racket[управляющий] по умолчанию ---
@racket[управляющий-ограничением-времени] с этим временем (секунды) для
экземпляра и продолжений; если @racketvalfont{ложь} --- управляющий с
порогом памяти 128~МБ. @racket[корень-сервера] --- каталог корня сервера
по умолчанию; от него считаются @racket[дополнительные-файлы],
@racket[корень-сервлетов] и ответы об ошибках.
@racket[путь-типов-содержимого] --- файл @filepath{mime.types} в корне
или в пакете сервера.
...
@defproc[#:kind "функция" (параметр-файла? [значение любой])
         булево?]{
Истина, если @racket[значение] --- загруженный файл из полей запроса.
Модуль @racketidfont{сервер-гипертекста/обработчик}.}

@defproc[#:kind "функция" (параметр-файла [идентификатор байты?]
         [имя-файла байты?]
         [заголовки список?]
         [содержимое байты?])
         любой]{
Собирает параметр загруженного файла: @racket[идентификатор] --- имя
поля формы, @racket[имя-файла] --- имя файла, @racket[заголовки] ---
список заголовков MIME-конверта, @racket[содержимое] --- байты файла.
Модуль @racketidfont{сервер-гипертекста/обработчик}.}

@defproc[#:kind "функция" (идентификатор-параметра-файла [параметр любой])
         байты?]{
Имя поля формы у загруженного файла. Модуль
@racketidfont{сервер-гипертекста/обработчик}.}

@defproc[#:kind "функция" (имя-файла-параметра [параметр любой])
         байты?]{
Имя загруженного файла. Модуль
@racketidfont{сервер-гипертекста/обработчик}.}

@defproc[#:kind "функция" (заголовки-параметра-файла [параметр любой])
         список?]{
Заголовки MIME-конверта загруженного файла. Модуль
@racketidfont{сервер-гипертекста/обработчик}.}

@defproc[#:kind "функция" (содержимое-параметра-файла [параметр любой])
         байты?]{
Байты загруженного файла. Если параметр создан через
@racket[параметр-файла/порт], первое обращение читает оставшееся
содержимое порта, запоминает его и закрывает порт. Модуль
@racketidfont{сервер-гипертекста/обработчик}.}

@defproc[#:kind "функция" (параметр-файла/порт? [значение любой])
         булево?]{
Истина, если @racket[значение] --- загруженный файл с содержимым
в порте ввода. Модуль @racketidfont{сервер-гипертекста/обработчик}.}

@defproc[#:kind "функция" (порт-параметра-файла [параметр любой])
         порт-ввода?]{
Порт ввода с содержимым файла. Только для значений, для которых
@racket[параметр-файла/порт?] истинен. Модуль
@racketidfont{сервер-гипертекста/обработчик}.}

@defproc[#:kind "функция" (параметр-файла/порт [идентификатор байты?]
         [имя-файла байты?]
         [заголовки список?]
         [порт порт-ввода?])
         любой]{
Как @racket[параметр-файла], но содержимое задаётся портом ввода, а не
байтами. Модуль @racketidfont{сервер-гипертекста/обработчик}.}

@subsection[#:tag "ref-форлеты"]{@racketidfont{сервер-гипертекста/форлеты}}

Форлет --- кусок HTML-формы вместе с разбором ответа. Форлеты можно
вкладывать друг в друга. Подключение:
@racketidfont{сервер-гипертекста/форлеты}.

@defform[#:kind "синтаксис" (форлет шаблон выражение)]{
Собирает форлет. @racket[шаблон] --- разметка: тег с содержимым,
например @racketidfont{div}. Поле вкладывают через запятую и оператор
@racket[=>]: слева форлет поля, справа --- имя результата.
@racket[выражение] вычисляется при разборе отправки и может
использовать эти имена.

@codeblock|{
форлет
  (div «Месяц:» ,(ввод-числа => месяц)
       «День:» ,(ввод-числа => день))
  список месяц день
}|

Оператор @racket[=>] в шаблоне форлета --- вложение поля, а не ветка
@racket[если].}

@defform[#:kind "синтаксис" (лес узел ...)]{
В шаблоне форлета --- несколько узлов разметки рядом, без общего тега.
Допустимо только внутри @racket[форлет] и @racket[форлет*].}

@defform[#:kind "синтаксис" (форлет* шаблон выражение)]{
Как @racket[форлет], но @racket[шаблон] вычисляется как выражение
разметки (квазицитата через @litchar{`}). Поле вкладывают оператором
@racket[=>*]. Имена в @racket[выражение] привязаны к спискам
результатов: один форлет может встретиться несколько раз.
...
@defform[#:kind "оператор" #:id =>* (вложенный =>* результат)]{
Вложение поля в @racket[форлет*]. Допустимо только внутри
@racket[форлет*].}

@defproc[#:kind "функция" (разметка-форлета [форлет любой])
         список?]{
Лес узлов разметки для показа форлета. Имена полей в атрибуте
@racketidfont{name} уникальны.}

@defproc[#:kind "функция" (обработать-форлет [форлет любой]
         [запрос любой])
         любой]{
Разбирает поля @racket[запрос] согласно @racket[форлет] и возвращает
результат выражения форлета.}

@defproc[#:kind "функция" (отправить/форлет [форлет любой]
         [#:метод метод любой 'отправка]
         [#:обернуть обернуть функция?])
         любой]{
Показывает форлет как страницу и возвращает разобранный результат
отправки. @racket[метод] --- @racket['отправка] или @racket['POST]
(отправка), @racket['получение] или @racket['GET] (получение).
@racket[обернуть] получает разметку формы и возвращает страницу.
По умолчанию заголовок страницы --- «Ввод формы».}

@defproc[#:kind "функция" (встроить-форлет [встроить функция?]
         [форлет любой])
         любой]{
Строит разметку формы с адресом продолжения из @racket[встроить]
(как у @racket[отправить/приостановить/маршрутизировать]).}

@defproc[#:kind "функция" (создать-ввод [нарисовать функция?])
         любой]{
Низкоуровневое поле: @racket[нарисовать] получает имя поля (строка)
и возвращает узел разметки. Результат разбора --- привязка поля или
@racketvalfont{ложь}.}

@defproc[#:kind "функция" (создать-ввод* [нарисовать функция?])
         любой]{
Как @racket[создать-ввод], но результат разбора --- список всех
привязок с этим именем.}

@defproc[#:kind "функция" (поле-ввода
         [#:тип тип строка? «text»]
         [#:значение значение
          (один-из байты? строка? #,(elem (racketvalfont "ложь")))
          #,(elem (racketvalfont "ложь"))]
         [#:наибольшая-длина наибольшая-длина
          (один-из целое-неотрицательное?
           #,(elem (racketvalfont "ложь")))
          #,(elem (racketvalfont "ложь"))]
         [#:только-чтение? только-чтение? любой
          #,(elem (racketvalfont "ложь"))]
         [#:атрибуты атрибуты список? пустой-список])
         любой]{
Поле @racketidfont{input}.}

@defproc[#:kind "функция" (текстовый-ввод
         [#:значение значение
          (один-из байты? строка? #,(elem (racketvalfont "ложь")))
          #,(elem (racketvalfont "ложь"))]
         [#:размер размер
...
@defproc[#:kind "функция" (ввод-пароля
         [#:значение значение
          (один-из байты? строка? #,(elem (racketvalfont "ложь")))
          #,(elem (racketvalfont "ложь"))]
         [#:размер размер
...
@defproc[#:kind "функция" (многострочный-ввод
         [#:атрибуты атрибуты список? пустой-список]
         [#:значение значение
          (один-из байты? строка? #,(elem (racketvalfont "ложь")))
          #,(elem (racketvalfont "ложь"))]
         ...
         [#:столбцы столбцы
          ...
          #,(elem (racketvalfont "ложь"))])
         любой]{
Поле @racketidfont{textarea}.}

@defproc[#:kind "функция" (флажок-формы
         [значение (один-из байты? строка?)]
         [отмечен? любой]
         [#:атрибуты атрибуты список? пустой-список])
         любой]{
Флажок формы.}

@defproc[#:kind "функция" (переключатель
         [значение (один-из байты? строка?)]
         [отмечен? любой]
         [#:атрибуты атрибуты список? пустой-список])
         любой]{
...
@defproc[#:kind "функция" (группа-переключателей [варианты любой]
         [#:атрибуты атрибуты функция?]
         [#:отмечен? отмечен? функция?]
         [#:представить представить функция?]
         [#:обернуть обернуть функция?])
         любой]{
Группа переключателей по последовательности @racket[варианты].
Результат --- выбранный элемент. @racket[атрибуты], @racket[отмечен?]
и @racket[представить] получают элемент; @racket[обернуть] --- разметку
поля и подпись.}

@defproc[#:kind "функция" (группа-флажков [варианты любой]
         [#:атрибуты атрибуты функция?]
         [#:отмечен? отмечен? функция?]
         [#:представить представить функция?]
         [#:обернуть обернуть функция?])
         любой]{
Группа флажков. Результат --- список выбранных элементов.}

@defproc[#:kind "функция" (кнопка-отправки
         [подпись (один-из байты? строка?)]
         [#:атрибуты атрибуты список? пустой-список])
         любой]{
Кнопка отправки формы.}

@defproc[#:kind "функция" (кнопка-сброса
         [подпись (один-из байты? строка?)]
         [#:атрибуты атрибуты список? пустой-список])
         любой]{
Кнопка сброса формы.}

@defproc[#:kind "функция" (загрузка-файла
         [#:атрибуты атрибуты список? пустой-список])
         любой]{
Поле загрузки файла.}

@defproc[#:kind "функция" (скрытое-поле
         [значение (один-из байты? строка?)]
         [#:атрибуты атрибуты список? пустой-список])
         любой]{
Скрытое поле.}

@defproc[#:kind "функция" (рисунок-формы
         [подпись (один-из байты? строка?)]
         [источник (один-из байты? строка?)]
         [#:высота высота
          (один-из целое-неотрицательное?
           #,(elem (racketvalfont "ложь")))
          #,(elem (racketvalfont "ложь"))]
         ...
         [#:атрибуты атрибуты список? пустой-список])
         любой]{
Рисунок в форме (@racketidfont{img}): @racket[подпись] --- текст
@racketidfont{alt}, @racket[источник] --- адрес изображения.}

@defproc[#:kind "функция" (кнопка-формы
         [тип (один-из байты? строка?)]
         [текст (один-из байты? строка?)]
         [#:отключена? отключена? любой
          #,(elem (racketvalfont "ложь"))]
         [#:значение значение
          (один-из байты? строка? #,(elem (racketvalfont "ложь")))
          #,(elem (racketvalfont "ложь"))]
         [#:атрибуты атрибуты список? пустой-список])
         любой]{
Кнопка @racketidfont{button} с @racket[тип] и видимым @racket[текст].}

@defproc[#:kind "функция" (множественный-выбор [варианты любой]
         [#:атрибуты атрибуты список? пустой-список]
         [#:несколько? несколько? любой
          #,(elem (racketvalfont "истина"))]
         [#:выбран? выбран? функция?]
         [#:представить представить функция?])
         любой]{
Список выбора. По умолчанию можно отметить несколько пунктов.
Результат --- список выбранных элементов.}

@defproc[#:kind "функция" (список-выбора [варианты любой]
         [#:атрибуты атрибуты список? пустой-список]
         [#:выбран? выбран? функция?]
         [#:представить представить функция?])
         любой]{
Список выбора одного элемента.}

@defproc[#:kind "функция" (обязательный [форлет любой])
         любой]{
Требует, чтобы поле было заполнено; иначе исключение. Результат ---
байты значения.}

@defproc[#:kind "функция" (с-умолчанием [умолчание байты?]
         [форлет любой])
         любой]{
Как @racket[обязательный], но при пустом поле подставляет
@racket[умолчание].}

@defproc[#:kind "функция" (в-строку [форлет любой]) любой]{
Переводит байты значения поля в строку.}

@defproc[#:kind "функция" (в-число [форлет любой]) любой]{
Переводит строку значения поля в число.}

@defproc[#:kind "функция" (в-символ [форлет любой]) любой]{
Переводит строку значения поля в символ.}

@defproc[#:kind "функция" (в-булево [форлет любой]) любой]{
Истина, если байты значения равны @litchar{on}.}

@defthing[#:kind "форлет" ввод-строки любой]{
Обязательное текстовое поле; результат --- строка.}

@defthing[#:kind "форлет" ввод-числа любой]{
Обязательное текстовое поле; результат --- число.}

@defthing[#:kind "форлет" ввод-символа любой]{
Обязательное текстовое поле; результат --- символ.}
