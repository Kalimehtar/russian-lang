...
@(require 1/lang scribble/example scribble/core scribble/racket (for-label 1/all-base))
...
Если на одной строке есть несколько элементов, разделённых пробельными знаками, то это список.
...
количество строк не добавляя скобок. Знак «$» показывает, что
элементы справа от него являются списком, который должен быть подставлен на место этого знака.
...
сделать оператором добавив перед и после него знаки «^». Например, @racket[(2 ^cons^ 3)]
...
Комментарий начинается с знаков «@racketcommentfont{-}@racketcommentfont{-}»
...
последовательность знаков «\"», для записи знака «\» --- «\\». Все остальные знаки
...
Выражения записываются в виде последовательности слов, разделённых пробельными знаками.
Слово может быть оператором, если состоит только из знаков «!#$%&⋆+./<=>?@"@"^~:*-» кроме «.»
и «...» или начинается и заканчивается на «^». Примеры операторов: +, -, ^пара^.
...
Если строка очень длинная, то можно перед переносом вставить знак «\», тогда перенос
...
или значений в одну строку, можно использовать оператор «;».
...
(eval:alts (unsyntax (racketvalfont "кусок \"три знака\"")) "три")
]
...
Синтакисис для идентификаторов масимально свободный. В них могут быть использованы любые знаки
кроме пробелов, скобок, кавычек, апострофов, точки с запятой, запятой, решётки, вертикальной черты
и обратной косой черты. Если очень надо, запретный знак можно экранировать обратной косой чертой.
Более того, можно вводить идентификатор между вертикальным чертами, тогда
допустимы вообще любые знаки кроме вертикальной черты.
...
к идентификатору @racket[отобразить] за исключением того, что они оба состоят из одинаковых знаков.

Фактически, символ хранит только строку со своим именем. В этом смысле смиволы и строки отличаются
...
Синтаксис Адины не определяется напрямуй в терминах потоков знаков. Вместо этого
синтаксис определяется двумя слоями:

@itemlist[(list @item{слой читателя, который превращает знаки в списки, символы и другие константы.}
@item{слой раскрывателя, который преобразовывает получившиеся списки и контстанты в выражения.})]
...
как целые числа или простые дроби. Такое же соглашение используется при чтении, не если необходимо,
перед числом можно написать @litchar{#e}, тогда число будет прочитано как точное или
@litchar{#i} --- тогда оно будет прочитано как неточное. Префиксы @litchar{#b}, @litchar{#o}
и @litchar{#x} позволяють вводить числа в двоичной, восьмеричной и шестнадцатеричной системах
счисления.
...
Вычисление, вкючающее неточные числа, возвращает неточный результат, так что неточность действует
на числа как зараза. Это, разумеется, не относится к использованию неточных чисел в логических
операциях. Процедуры @racket[точное->неточное] и @racket[неточное->точное] позволяют преобразовывать
...
Если вызвана с одним элементом, то возвращает постоянное значение,
 ...
 Если с несколькими, то формрует из них список. Возвращаемое значение всегда
 неизменяемое.}
...
@defproc[#:kind "функция" (точное-число? [аргумент любой])
         булево?]{Возвращает истину, если @racket[аргумент] является точным числом.}

@defproc[#:kind "функция" (неточное-число? [аргумент любой])
...
@defproc[#:kind "функция" (неточное->точное [число число?])
         точное-число?]{Преобразовывает число в точное. Если @racket[число] одно из +inf.0, -inf.0,
 +nan.0, +inf.f, -inf.f или +nan.f, тогда вызывается исключение.}

@defproc[#:kind "функция" (точное->неточное [число число?])
         неточное-число?]{Преобразовывает число в неточное.}
...
@defproc[#:kind "функция" (длина-строки [строка строка?])
         целое-неотрицательное?]{Возвращает длину строки в знаках.}
...
@defproc[#:kind "функция" (строка-начинается-с? [строка строка?] [подстрока строка?])
         булево?]{Возвращает истину, если @racket[строка] начинается с знаков
 в аргументе @racket[подстрока].}

@defproc[#:kind "функция" (строка-заканчивается-на? [строка строка?] [подстрока строка?])
         булево?]{Возвращает истину, если @racket[строка] заканчивается на знаки
 в аргументе @racket[подстрока].}

@defproc[#:kind "функция" (прописные  [строка строка?])
         строка?]{Возвращает строку, в которой все знаки заменены на прописные.}
