...
Оператор @racket[?] имеет три параметра и поэтому обрабатывается особым образом. Слова слева от него
...
(выражение должно в этом случае возвращать необходимое количество значений).
 ...
 то создаётся функция, возвращающая функцию с параметрами, указанными после первого элемента
 заголовка.}

@defform*[#:kind "синтаксис" #:id функция
   ((функция (параметры) = команда ... выражение))
   #:grammar [(параметры (code:line параметр ...)
                         (code:line параметр ... @#,racketparenfont{.} параметр-оставшихся))
              (параметр идентификатор [идентификатор выражение] (code:line ключ идентификатор)
                        (code:line [ключ идентификатор выражение]))
              ]]{
   Возвращает функцию с указанными параметрами и телом.}
...
@defproc[#:kind "функция" (/= [параметр любой] ...+)
         логический?]{Возвращает @racketvalfont{ложь}, если @racket[параметр]ы
