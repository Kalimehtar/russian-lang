...
системы таковыми являются числа -1152921504606846976..1152921504606846975, но конкретные границы
могут зависеть от используемой платформы. Для конкретного компьютера можно проверять при помощи
функции @racket[небольшое-число?].
...
@examples[#:label #f
 ...
 (eval:alts
  (unsyntax (elem (racket замерить-время $ округлить $ сумма (функция (ч) $ 1.0 / ч) 1 2000)
                  (linebreak) (racketoutput "время ЦП: 24 реальное: 12 мусорщика: 0")))
  (void))
 (eval:alts
  (unsyntax (elem (racket замерить-время $ округлить $ сумма (функция (ч) $ 1 / ч) 1 2000)
                  (linebreak) (racketoutput "время ЦП: 0 реальное: 0 мусорщика: 0")))
  (void))]
...
@defproc[#:kind "функция" (целое? [аргумент любой])
 ...
 Внимание, неточное чило тоже может быть целым.}
