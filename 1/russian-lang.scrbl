...
@defterm{Числа} могут быть записаны как целые произвольной длины, в виде десятичных или простых дробей, с экспонентой или мнимой частью.
@racketblock[
10      2.5
1/3     1.02e+13
...
@defterm{Строки} записываются между двойными кавычками. Для записи кавычк используется последовательность символов «\"», для записи символа «\» --- «\\». Все остальные символы
...
если состоит только из символов «!#$%&⋆+./<=>?@"@"^~:*-» или начинается и заканчивается на «^». Примеры операторов: +, -, ^пара^.
...
@examples[#:label "Примеры:"
(eval:alts (unsyntax (racketvalfont "список 1 2 3")) '(1 2 3))
(eval:alts (unsyntax (racketvalfont "пара 5 6")) '(5 . 6))
]
...
(eval:alts (unsyntax (racketvalfont "список 1 2 3 4\n    список 5 6\n    7; список 8; 9")) '(1 2 3 4 (5 6) 7 (8) 9))
]
