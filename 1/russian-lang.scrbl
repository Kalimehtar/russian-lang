...
Например, функция @racket[создать-хеш] использует список пар,
...
@examples[#:label #f
 (eval:alts
  (unsyntax (elem (racket буквально 1 . 2)))
  '(1 . 2))
 (eval:alts
  (unsyntax (elem (racket буквально 0 . (1 . 2))))
  '(0 1 . 2))]
...
@subsection[#:tag "hash"]{{Хеш-таблицы}}

@defproc[#:kind "функция" (хеш? [аргумент любой])
         логический?]{Возвращает истину, если @racket[аргумент] является хеш-таблицей.}

@defproc[#:kind "функция" (создать-хеш [ассоциативный-список (список-из пара?)])
         хеш?]{Возвращает хеш-таблицу, трактуя элементы из списка как пары @racket[ключ : значение].}
