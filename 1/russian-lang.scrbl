...
@racket[переместить], @racket[установить-размер],
...
@defproc[#:kind "метод" (нарисовать-текст [текст строка?] [лево число?] [верх число?]
                  [комбинировать любой #f] [начальная-литера целое? 0]
                  [угол число? 0])
         пусто?]{
...
@defproc[#:kind "метод" (нарисовать-изображение [источник изображение%] [лево-цели число?]
                        ...
                        [цвет любой 'чёрный] [маска любой #f])
         булево?]{
...
@defproc[#:kind "конструктор" #:link-target? #f (изображение% [ширина любой #f]
               [высота любой #f]
               [масштаб число? 1.0]
               [файл любой #f]
...
@defproc[#:kind "метод" (скопировать [дополнять? любой ложь] [время число? 0])
         пусто?]{
...
@defproc[#:kind "конструктор" #:link-target? #f (главное-окно% [заголовок строка? ""]
               ...
               [минимальная-ширина любой #f]
                                  [минимальная-высота любой #f]
...
@defproc[#:kind "функция" (получить-файл [сообщение (один-из строка? ложь) ложь]
                               [родитель (один-из главное-окно% диалог% ложь) ложь]
                               [каталог (один-из строка? ложь) ложь]
                               [имя-файла (один-из строка? ложь) ложь]
                               [расширение (один-из строка? ложь) ложь]
...
@racketvalfont{ложь}, если пользователь отменил выбор. @racket[сообщение] --- текст в верхней
части диалога. @racket[родитель] --- окно-родитель. @racket[каталог] и @racket[имя-файла]
задают начальный каталог и имя. @racket[расширение] --- расширение по умолчанию.
@racket[стиль] --- список флагов (@racket['common], @racket['packages],
@racket['enter-packages]). @racket[фильтры] --- список пар (подпись, маска).
Модуль @racketidfont{графический-интерфейс/диалог}.}
...
@defproc[#:kind "конструктор" #:link-target? #f
         (текст% [межстрочный-промежуток число? 1.0]
                 [автоматический-перенос любой #f])
         текст%]{
Создаётся формой @racket[объект]:
@racketblock[
объект текст%
