...
@defproc[#:kind "конструктор" #:link-target? #f (цвет% [красный целое-неотрицательное? 0]
...
Если задан @racket[внутренний] или @racket[образец], составляющие игнорируются. Без образца и без
внутреннего объекта создаётся цвет из составляющих; укрывистость по умолчанию 0 --- цвет полностью
прозрачный, пока её не задать явно.}

@defthing[#:kind "класс" холст% класс?]{
Базовый класс: @racket[объект-графического-интерфейса%].
Контекст рисования: линии, фигуры, текст и изображения. Холст картины даёт метод @racketidfont{холст};
у @racket[изображение%] --- тоже @racketidfont{холст}. Модуль @racketidfont{рисование/холст}
предоставляет @racket[холст-адины]; объекты, которые она возвращает, принадлежат этому классу.
...
@defproc[#:kind "метод" (залить-фоном)
         пусто?]{
заполняет область текущим цветом фона. См. также @racketidfont{очистить}.}
...
@defproc[#:kind "метод" (копировать)
         пусто?]{
копирует прямоугольник (@racket[лево], @racket[верх], @racket[ширина], @racket[высота]) в точку
(@racket[лево-цели], @racket[верх-цели]) на том же холсте. Области могут пересекаться.}

@defproc[#:kind "метод" (нарисовать-дугу)
         пусто?]{
...
@defproc[#:kind "метод" (нарисовать-эллипс)
         пусто?]{
эллипс в прямоугольнике лево--верх--ширина--высота. Перо --- контур, кисть --- заливка.}

@defproc[#:kind "метод" (нарисовать-прямоугольник)
         пусто?]{
прямоугольник с тем же смыслом пера и кисти.}

@defproc[#:kind "метод" (нарисовать-закруглённый-прямоугольник)
         пусто?]{
прямоугольник со скруглёнными углами. Аргумент @racket[радиус] по умолчанию $-0.25$: положительный ---
радиус дуги угла, отрицательный --- доля меньшей стороны. Если величина слишком велика, берётся
половина ширины или высоты.}

@defproc[#:kind "метод" (нарисовать-линию)
         пусто?]{
отрезок от (@racket[лево-начала], @racket[верх-начала]) до (@racket[лево-конца],
@racket[верх-конца]).}

@defproc[#:kind "метод" (нарисовать-линии)
         пусто?]{
ломаная по списку точек. Необязательные @racket[сместить-вправо] и @racket[сместить-вниз] (по
умолчанию 0) сдвигают все точки.}

@defproc[#:kind "метод" (нарисовать-многоугольник)
         пусто?]{
многоугольник по списку точек. Смещения как у линий. @racket[стиль-заполнения] по умолчанию
@racket['чёт-нечет]; иное значение --- правило ненулевой обмотки.}

@defproc[#:kind "метод" (нарисовать-контур)
         пусто?]{
рисует контур. Смещения и @racket[стиль-заполнения] как у многоугольника.}

@defproc[#:kind "метод" (нарисовать-точку)
         пусто?]{
точка в (@racket[лево], @racket[верх]).}

@defproc[#:kind "метод" (нарисовать-лекало)
         пусто?]{
сплайн от начала до конца через контрольную точку. Каждая точка --- список из лево и верх или объект с
полями @racketidfont{лево} и @racketidfont{верх}.}

@defproc[#:kind "метод" (нарисовать-текст)
         пусто?]{
строка текущим шрифтом. Аргументы: текст, лево, верх; @racket[комбинировать] (по умолчанию
@racketvalfont{ложь} --- литеры рисуются по отдельности); @racket[начальная-литера] (с какой литеры
начать, по умолчанию 0); @racket[угол] в радианах против часовой стрелки (по умолчанию 0). Если
@racket[комбинировать] --- @racket['графемы], соседние литеры одного знака рисуются вместе; любое
другое истинное значение может включать лигатуры, кернинг и письмо справа налево.}

@defproc[#:kind "метод" (нарисовать-изображение)
         булево?]{
рисует @racket[изображение%] в (@racket[лево-цели], @racket[верх-цели]). @racket[стиль] (по умолчанию
@racket['обычный]) и @racket[цвет] (по умолчанию чёрный) учитываются только для чёрно-белого
изображения: @racket['непрозрачный] закрашивает белые пиксели фоном. @racket[маска] --- другое
изображение того же размера или @racketvalfont{ложь}. Возвращает @racketvalfont{истина}, если рисунок
удался.}

@defproc[#:kind "метод" (нарисовать-часть-изображения)
         булево?]{
как @racketidfont{нарисовать-изображение}, но копирует прямоугольник источника (@racket[лево-части],
@racket[верх-части], @racket[ширина-части], @racket[высота-части]).}

@defproc[#:kind "метод" (начать-слой)
         пусто?]{
...
@racketidfont{прозрачность}, перекрывающиеся штрихи слоя тускнеют вместе, а не по отдельности.}

@defproc[#:kind "метод" (закончить-слой)
         пусто?]{
завершает слой, начатый @racketidfont{начать-слой}.}

@defproc[#:kind "метод" (отрисовать-немедленно)
         пусто?]{
сбрасывает отложенную отрисовку картины; для прочих холстов ничего не делает.}

@defproc[#:kind "метод" (начать-документ)
         пусто?]{
начало печати: сообщение к началу документа.}
...
цвет фона для @racketidfont{залить-фоном} (чтение и запись).}
...
Нужно задать либо @racketidfont{файл}, либо оба @racketidfont{ширина} и @racketidfont{высота}, либо
@racketidfont{внутренний}. Иначе вызывается исключение с сообщением, что ширина и высота обязательны.
...
@defproc[#:kind "конструктор" #:link-target? #f (изображение% [ширина любой]
               [высота любой]
               [масштаб число? 1.0]
               [файл любой]
               [вид-файла любой]
               [цвет-фона любой]
               [неудачная-загрузка-ошибка? булево? #f]
               [хранить-данные-файла? булево? #f]
               [байты любой]
               [чёрно-белое? любой]
               [прозрачность? булево? #f]
               [внутренний любой])
         изображение%]{
Создаётся формой @racket[объект]:
@racketblock[
...
@racket[ширина] и @racket[высота] обязательны вместе, если нет файла и нет внутреннего объекта.
@racket[вид-файла] по умолчанию определяется по содержимому. @racket[неудачная-загрузка-ошибка?]
по умолчанию @racketvalfont{ложь}. @racket[байты] --- чёрно-белый растр из битов (восемь пикселей в
байте, 1 --- чёрный).}
...
при рисовании не применяется: её передают как @racketidfont{маска} в
@racketidfont{нарисовать-изображение}.}

@defproc[#:kind "метод" (пиксели-в-байты [лево число?] [верх число?] [ширина число?]
                 [высота число?] [байты любой]
                 [только-прозрачность? булево? #f]
                 [учитывать-прозрачность? булево? #f]
                 [без-масштаба? булево? #f])
         пусто?]{
...
@defproc[#:kind "метод" (байты-в-пиксели [лево число?] [верх число?] [ширина число?]
                 [высота число?] [байты любой]
                 [только-прозрачность? булево? #f]
                 [учитывать-прозрачность? булевo? #f]
                 [без-масштаба? булевo? #f])
         пусто?]{
...
@defproc[#:kind "метод" #:link-target? #f (внутренний)
         любой]{
Возвращает внутренний объект, обёрнутый этим классом.}
...
Аргументы инициализации: @racketidfont{граница}, @racketidfont{интервал}, @racketidfont{выравнивание}
--- список из двух символов (у каждого класса своё умолчание).
}
...
@defproc[#:kind "метод" (начать-изменения)
         пусто?]{
Начало пакета обновлений раскладки: между ним и @racketidfont{закончить-изменения} дети не
пересчитываются.}
...
@defproc[#:kind "метод" (установить-иконку [иконка любой] [маска любой ложь]
                    [какую любой 'обе])
         пусто?]{
...
@defproc[#:kind "метод" (можно-выйти?)
         булево?]{
Разрешить ли выход из приложения; по умолчанию вызывает @racketidfont{можно-закрыть?}.}
...
@racket[шрифт-элемента-управления]), @racketidfont{стиль} --- список символов; @racket['скрытая] ---
элемент создан, но не показан в родителе.
}
...
@racketidfont{при-получении-клавиши} окна верхнего уровня, а не доходят до картины.}
...
@defproc*[#:kind "метод" #:link-target? #f
          ([(фон) любой]
           [(фон [цвет цвет%]) пусто?])]{
Без аргумента возвращает цвет, которым стирают картину перед @racketidfont{при-отрисовке}. С
аргументом задаёт этот цвет.
...
стольких же вызовов @racketidfont{продолжить-отрисовку}.}

@defproc[#:kind "метод" (продолжить-отрисовку)
         пусто?]{
Возобновляет сброс буфера после @racketidfont{приостановить-отрисовку}. См.
@racketidfont{приостановить-отрисовку}.}
...
Вызывается, когда картину нужно перерисовать (показ, изменение размера, запрос
@racketidfont{обновить}). Если вернуть не @racketvalfont{ложь}, стандартная отрисовка (в том числе
@racketidfont{обработка}) не выполняется.
...
@racketidfont{при-изменении-фокуса} тоже вызывается). Если вернуть не @racketvalfont{ложь},
стандартная обработка не выполняется.
В базовом классе возвращает @racketvalfont{ложь}.}
...
@defproc[#:kind "метод" (закончить-последовательность-редактирования)
         пусто?]{
Завершение набора правок, начатого @racketidfont{начать-последовательность-редактирования}.}
...
позиции 0. Остальные куски обходятся через @racketidfont{следующий}.}
...
@defproc[#:kind "метод" (глобальные->местные [лево число?] [верх число?])
         любой]{
Переводит координаты @racketidfont{лево} и @racketidfont{верх} из системы показа (обычно картины) в
координаты документа. Возвращает два значения: местные лево и верх.}
...
@defproc[#:kind "метод" (обработать-изменение-размера)
         пусто?]{
Вызывает @racketidfont{при-изменении-размера}, если документ не в середине последовательности
редактирования и не перерисовывается сейчас. Иначе вызов откладывается.}
...
Показ изменил размер видимой области (как у @racketidfont{размеры}). Вызывается не напрямую, а через
@racketidfont{обработать-изменение-размера}. В базовом классе ничего не делает. Если включён
автоматический перенос, исходная реализация подстраивает максимальную ширину под картины документа.}
...
@defproc[#:kind "конструктор" #:link-target? #f (главное-окно% [заголовок строка? ""]
               [ширина любой #f]
               [высота любой #f]
               [родитель любой]
               ...
               [минимальная-ширина любой]
                                  [минимальная-высота любой]
                                  [растягивается-ширина булево? #t]
                                  [растягивается-высота булево? #t]
...
@defproc[#:kind "конструктор" #:link-target? #f (диалог% [заголовок строка? ""]
          [ширина любой #f]
          [высота любой #f]
          [родитель любой]
          [граница целое-неотрицательное? 0]
...
@defproc[#:kind "конструктор" #:link-target? #f (группа% [родитель любой]
          [граница целое-неотрицательное? 0]
                             [интервал целое-неотрицательное? 0]
                             [выравнивание любой '(центр центр)]
          [минимальная-ширина любой]
...
@defproc[#:kind "конструктор" #:link-target? #f (горизонтальная-группа% [родитель любой]
          [граница целое-неотрицательное? 0]
                             [интервал целое-неотрицательное? 0]
                             [выравнивание любой '(центр центр)]
          [минимальная-ширина любой]
...
@defproc[#:kind "конструктор" #:link-target? #f (вертикальная-группа% [родитель любой]
          [граница целое-неотрицательное? 0]
                             [интервал целое-неотрицательное? 0]
                             [выравнивание любой '(центр центр)]
          [минимальная-ширина любой]
...
@defproc[#:kind "конструктор" #:link-target? #f (панель% [родитель любой]
          [стиль любой '()]
          [граница целое-неотрицательное? 0]
                             [интервал целое-неотрицательное? 0]
                             [выравнивание любой '(центр центр)]
...
@racket['(центр центр)]. @racket[родитель] обязателен.}
...
@defproc[#:kind "конструктор" #:link-target? #f (горизонтальная-панель% [родитель любой]
          [стиль любой '()]
          [граница целое-неотрицательное? 0]
                             [интервал целое-неотрицательное? 0]
                             [выравнивание любой '(центр центр)]
...
@defproc[#:kind "конструктор" #:link-target? #f (вертикальная-панель% [родитель любой]
          [стиль любой '()]
          [граница целое-неотрицательное? 0]
                             [интервал целое-неотрицательное? 0]
                             [выравнивание любой '(центр центр)]
...
@defproc[#:kind "конструктор" #:link-target? #f (табличная-панель% [родитель любой]
                    [стиль любой '()]
                    [измерения любой '(1 1)]
...
@defproc[#:kind "конструктор" #:link-target? #f (кнопка% [заголовок любой]
          [родитель любой]
          [команда функция? (функция (кнопка событие-управления%) пусто)]
          [шрифт шрифт%]
                             [стиль любой '()]
                             [растягивается-ширина булево? #f]
                             [растягивается-высота булево? #f]
          [минимальная-ширина любой]
                             [минимальная-высота любой]
          [горизонтальные-поля целое-неотрицательное? 0]
                             [вертикальные-поля целое-неотрицательное? 0]
...
@racket['многострочная], @racket['скрытая].}
...
@defproc[#:kind "конструктор" #:link-target? #f (надпись% [заголовок любой]
           [родитель любой]
           [цвет любой #f]
           [размер-по-заголовку булево? #f]
           [шрифт шрифт%]
                              [стиль любой '()]
                              [растягивается-ширина булево? #f]
                              [растягивается-высота булево? #f]
...
@racketvalfont{ложь}. @racket[размер-по-заголовку] по умолчанию @racketvalfont{ложь}.}
...
@defproc[#:kind "конструктор" #:link-target? #f (текстовое-поле% [родитель любой]
                  [заголовок любой #f]
                  [значение строка? ""]
                  [при-изменении функция? #f]
...
@racket['(однострочное)], @racket['многострочное] или @racket['скрытая]. @racket[растягивается-ширина]
по умолчанию @racketvalfont{истина}. При правке вызывается @racketidfont{при-изменении}; тип события
...
@defproc[#:kind "конструктор" #:link-target? #f (поле-даты% [родитель любой]
              [заголовок любой #f]
              [значение строка? ""]
              [при-изменении функция? #f]
              [стиль любой '(однострочное)]
...
Показывают методом @racketidfont{контекстное-меню} окна. Пункты --- @racket[пункт-меню%].}
...
@defproc[#:kind "конструктор" #:link-target? #f (пункт-меню% [родитель любой]
              [заголовок любой]
...
@defproc[#:kind "конструктор" #:link-target? #f (шрифт% [размер число? 12]
         [семейство любой 'default]
         [имя любой #f]
         [стиль символ? 'нормальный]
         [насыщенность символ? 'нормальная]
         [подчёркнутый? булево? #f]
         [сглаживание символ? 'стандартное]
         [размер-в-пикселях? булево? #f]
         [округлять-метрики? булево? #t]
...
@defproc[#:kind "метод" (есть-литера? [литера любой] [для-заголовка? булево? #f])
         булево?]{
...
@defproc[#:kind "конструктор" #:link-target? #f (событие-мыши% [тип символ? 'вход]
                [время целое? 0]
                [внутренний любой])
         событие-мыши%]{
Создаётся формой @racket[объект]:
...
@defproc[#:kind "конструктор" #:link-target? #f (поле-редактора% [родитель любой]
                   [заголовок любой #f]
                   [редактор любой #f]
                   [стиль любой '()]
                   [горизонтальные-поля целое-неотрицательное? 0]
...
Базовый класс: @racket[объект-графического-интерфейса%].
Фрагмент документа. Модуль @racketidfont{графический-интерфейс/кусок}. Создаётся без обязательных
аргументов.
}
...
@defproc[#:kind "метод" (скопировать)
         любой]{
...
@defproc[#:kind "конструктор" #:link-target? #f (кусок-с-изображением% [содержимое любой #f])
...
@defproc[#:kind "конструктор" #:link-target? #f (кусок-с-редактором% [редактор любой #f]
                       [есть-граница булево? #t])
         кусок-с-редактором%]{
Создаётся формой @racket[объект]:
@racketblock[
...
Базовый класс: @racket[объект-графического-интерфейса%].
Отличие стиля от базового при @racketidfont{изменить-стиль}. Создаётся без аргументов (пустое
отклонение).
}
...
@defproc[#:kind "функция" #:link-target? #f (набор-команд-адины [внутренний любой])
         набор-команд%]{
Оборачивает внутренний набор команд в @racket[набор-команд%].}
@defproc[#:kind "функция" #:link-target? #f (набор-команд-адины [внутренний любой])
         набор-команд%]{
