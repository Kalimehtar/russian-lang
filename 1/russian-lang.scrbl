...
Если элемент состоит из одного слова, он является занчением, если же из нескольких,
...
ввод заканчивает после пустой строки, так как до этого возможно продолжение команды.
...
писать, а просто перечислить параметры через пробел.

@examples[#:label "Примеры:"
(eval:alts (unsyntax (racketvalfont "часть = 3")) (void))
(eval:alts (unsyntax (elem (racketvalfont "кусок строка =\n    ") (racket подстрока)
...
На самом деле, определение функции также как и определение не функции всего лишь связывает
...
(выражение должно в этом случае возвращать необходимое количество значений).
 Третья связывает идентификатор с функцией.}
