...
;; аргументы-вызова синтаксис - если после правил операторов содержимое скобок
;;   стало вызовом оператора, оборачивает его, чтобы f(1 + 2) → (f (+ 1 2)),
;;   а не (f + 1 2). Иначе список аргументов подставляется как есть.
;; call-args syntax - when operator rules turned paren contents into an operator
;;   application, wrap it so f(1 + 2) → (f (+ 1 2)), not (f + 1 2).
;; : syntax? -> (or/c syntax? list?)
(define (аргументы-вызова stx)
  (define элементы (syntax-e stx))
  (if (and (pair? элементы) (оператор? (car элементы)))
      (list stx)
      stx))
    ...
    [(литеры-равны? литера #\»)
     (cond
       ...
       [(= уровень 0)
        (read-syntax
         (current-source-name)
         (open-input-string (string-append "\"" (list->string (reverse результат)) "\"")))]
       [else
       ...
       (let loop ([res res])
         ...
         (cond
           [(литеры-равны? следующий-символ #\()
            (read-char)
            (loop (datum->syntax #f (cons res (аргументы-вызова
                                               (прочитать-список-с-правилами #\))))
                                 (позиция)
                                 res))]
           [(литеры-равны? следующий-символ #\[)
...
(module+ test
  ...
  (test "f(1 + 2)" '(f (+ 1 2)))
  (test "ааа(положение + скорость())" '(ааа (+ положение (скорость))))
  (test "f(g(x) + 1)" '(f (+ (g x) 1)))
